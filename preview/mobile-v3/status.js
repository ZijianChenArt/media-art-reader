(() => {
  ...
  function buildMobileFacts(card) {
    ...
    appendFact(facts, 'FUNDING / 支持', support);
    appendFact(
      facts,
      'TRAVEL+STAY / 差旅',
      compactText(
    ...
    appendFact(facts, 'FIT / 适合', compactText(suitable, 44));
  ...
  cards.forEach(card => {
    ...
    if (deadlineNote) deadlineNote.textContent = 'APPLICATION DEADLINE / 截止';
    ...
    if (apply && !apply.querySelector('.apply-copy')) {
      ...
      label.textContent = 'OFFICIAL SOURCE / 官方页面';

      const host = document.createElement('span');
      host.className = 'apply-domain';
      host.textContent = domain ? '//' + domain : '//PRIMARY_SOURCE';
