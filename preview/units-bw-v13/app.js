(() => {
  ...
  function renderCard(item,i){
    ...
    const fit=item.fit||item.eligibility||'';
    return `<article class="call-card">
      ...
      <details class="card-more">
        ...
        <div class="more-grid">
          <div class="more-item"><small>PROJECT</small><p>${esc(item.project_when||item.program_dates||'—')}</p></div>
          <div class="more-item"><small>TRAVEL / STAY</small><p>${esc(short(travel,150)||'—')}</p></div>
          <div class="more-item"><small>APPLICATION FEE</small><p>${esc(short(item.application_fee,90)||'—')}</p></div>
          <div class="more-item"><small>ELIGIBILITY / FIT</small><p>${esc(short(fit,180)||'—')}</p></div>
        </div>
      </details>
  ...
  function opportunitiesView(filter='all'){
    ...
    const filterOrder=['all','exhibition','residency','prize','conference'];
    const filterText={all:'全部',...labels};
    return `<section class="view">
      ...
      <div class="opportunities-toolbar">
        <div class="mobile-filters">
          ${filterOrder.map(key=>`<button class="mobile-filter ${filter===key?'is-active':''}" type="button" data-mobile-filter="${key}">${esc(filterText[key])}</button>`).join('')}
        </div>
  ...
  function render(route,filter='all',push=false){
    ...
    $('.mobile-filter').forEach(btn=>{
      btn.addEventListener('click',()=>render('opportunities',btn.dataset.mobileFilter||'all',true));
    });
  }
