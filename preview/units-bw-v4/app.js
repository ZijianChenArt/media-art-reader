...
/* V4 scrollspy: selected nav item is hollow/white. */
(() => {
  const links=[...document.querySelectorAll('.rail-stack a[data-section]')];
  const main=document.querySelector('.main');
  if(!links.length||!main) return;
  const activate=id=>{
    links.forEach(a=>a.classList.toggle('is-active',a.dataset.section===id));
  };
  const sections=['calls','focus'].map(id=>document.getElementById(id)).filter(Boolean);
  const io=new IntersectionObserver(entries=>{
    const visible=entries.filter(e=>e.isIntersecting).sort((a,b)=>b.intersectionRatio-a.intersectionRatio)[0];
    if(visible) activate(visible.target.id);
  },{root:main,rootMargin:'-18% 0px -58% 0px',threshold:[0,.15,.35,.6]});
